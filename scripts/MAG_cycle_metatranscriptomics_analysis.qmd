...
```
## Data 
This analysis focuses on the MAG `r params$mag_id`` which represents the Geobacter genus. We hereafter refer to this as "Geobacter MAG". For this analysis, we require the following information:
...
bakta2MAG_path <- '/ibex/user/naras0c/membrane_cleaning/output/metatranscriptomics/quantification/bin2bakta_id_mappings.tsv'
...
annotation_folder <- '/ibex/user/naras0c/membrane_cleaning/output/annotation/all/bakta/'
...
that we evaluated (@fig-hkg_expression_conditions), which may not make them
suitable to be used as a reference for scaling. @tbl-hkg_genes_count summarises
the variability of HKGs across all samples. However, if we must make a choice,
we select the HKG with the lowest coefficient of variation (cv) as the reference
...
p.pca[[1]] <- 
  ...
  ggplot(aes(x = PC1, y = PC2, colour = as.character(condition), shape = phase)) +
  geom_point() +
...
p.mds[[1]] <- 
  ...
  pivot_sample() %>% as_tibble() %>% 
  ggplot(aes(x = Dim1, y = Dim2, colour = as.character(condition), shape = phase)) +
  geom_point() +
  geom_text_repel(aes(label = .sample), show.legend = FALSE) +
  theme_minimal()
```
...
solely based on condition in the raw counts and in the scaled/normalised
