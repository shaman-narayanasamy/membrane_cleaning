...
`params$cycle_id` which represents taxa with the following information:
```{r load_MAG_information}
read_tsv("/Users/shaman.narayanasamy/Work/Data/KAUST/membrane_cleaning/dereplicated_refined_MAG_information.tsv") %>% 
...
multi-dimensional scaling (MDS; @fig-mds) show the clustering of samples based
on condition in the raw counts and in the different scaled/normalised. The
boxplots (@fig-boxplot) display the effect of the various normalisation and
scaling procedures applied to the raw counts. The density plots
(@fig-density_plot) shows the distrubution of the raw and scaled counts.
...
write_tsv(de_dat, 
          paste0("/Users/shaman.narayanasamy/Work/Data/KAUST/membrane_cleaning/", 
                 mag_id, 
                 "-cycle_",  cycle_id, 
                 "-", phase_id,
...
de_dat %>% 
  ...
  write_tsv(paste0("/Users/shaman.narayanasamy/Work/Data/KAUST/membrane_cleaning/", 
                   mag_id, 
                   "-cycle_", cycle_id, 
                   "-", phase_id,
...
```
```{r function_stripchart}
plot_violin_stripchart <- function(annotation_dat, gene_list, gene_pattern, count_dat, comp_de_dat){
 
  annot_dat <- 
    annotation_dat %>% 
    filter(grepl(pattern, gene_function, ignore.case = TRUE)) %>% 
    inner_join(comp_de_dat, by = "feature") %>% 
    left_join(count_dat %>% as_tibble(), by = "feature") %>% 
  ...
  annotation_dat %>% 
    ...
    facet_wrap(~gene_name, ncol = 3) %>% return()
}
...
#| fig-height: 7
if(!exists("comp_de_norm")){ 
  print("No values aye")
  } else { 
    plot_violin_stripchart(MAG_annotation, biofilm_genes, keywords, counts, comp_de_norm)
  }
```
