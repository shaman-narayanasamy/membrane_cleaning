...
MAG_annotation <- 
  sprintf('ssh ibex "cat %s"', paste(annotation_folder, MAGs_of_interest, paste0(MAGs_of_interest, ".tsv"), sep = '/')) %>%  
  pipe() %>%   
...
counts <- 
  ...
  select(feature = Name, sample = sample_ID, counts, SampleName, condition, phase, cycle, lane, Run = sample_ID, avgLength = EffectiveLength) %>% 
...
combat_counts <- 
  ComBat_seq(counts(dds), 
             batch = colnames(dds) %>% as_tibble() %>% rename(sample = value) %>% left_join(sample_info, by = c("sample" = "sample_ID")) %>% pull(cycle),
...
MAG_annotation %>% 
  ...
  theme_minimal()
```
```{r visualise_hkg_cycles}
#| fig-cap: "Expression of housekeeping genes within cycles based on raw counts."
#| label: fig-hkg_expression_cycle
MAG_annotation %>% 
  filter(gene_symbol %in% housekeeping_genes) %>% 
  left_join(counts %>% as_tibble(), by = "feature") %>% 
  mutate(cycle = as.character(cycle)) %>% 
  # make faceted stripchart
  ggplot(aes(x = cycle, y = counts + 1, fill = cycle, group = cycle)) +
  geom_boxplot() +
  geom_jitter() +
  facet_wrap(~gene_symbol) +
  scale_y_log10() +
  theme_minimal()
```
```{r hkg_cycle_tables, message=F, warning=F, eval = F}
housekeeping_cycles <- list()
for (cycle_id in unique(sample_info$cycle)) {
 
   housekeeping_cycles[[cycle_id]] <-  
     MAG_annotation %>%  
     filter(gene_symbol %in% housekeeping_genes) %>% 
     left_join(counts %>% as_tibble(), by = "feature") %>% 
     filter(cycle == cycle_id) %>% 
     group_by(feature, sample) %>% 
     summarize(
       avg_count = mean(counts, na.rm = TRUE)
     ) %>%
...
#### Inspect housekeeping genes within each cycle
```{r hkg_cycles, message=F, warning=F}
housekeeping_cycles <- list()
...
and cycle (@fig-hkg_expression_cycles) -level expression, which may not
...
p.pca[[1]] <- 
  ...
  ggplot(aes(x = PC1, y = PC2, colour = as.character(cycle), shape = phase)) +
  geom_point() +
...
p.mds[[1]] <- 
  ...
  pivot_sample() %>% as_tibble() %>% 
  ggplot(aes(x = Dim1, y = Dim2, colour = as.character(cycle), shape = phase)) +
  geom_point() +
  geom_text_repel(aes(label = .sample), show.legend = FALSE) +
  theme_minimal()
```
...
p.bp[[1]] <-  
  counts %>% as_tibble() %>%  
  ggplot(aes(x = .sample, y = log(counts), fill = as.character(cycle))) +
  geom_boxplot() 
```
...
solely based on cycle number in the raw counts and in the scaled/normalised
...
The model applied for DE tests is `~condition + cycle + lane`. Specifically,
we compute the contrast between the true biological conditions of interest (i.e.
treatment vs control) while "removing" technical differences `cycle` and
  ...
  for (i in 1:length(expr_list)){ 
    ...
    tmp_de_res <-  
      ...
      test_differential_abundance(
        ~ 0 + condition + cycle + lane,
        method = "edgeR_quasi_likelihood",
        prefix = paste0(expr_source, "_", "edgerQLT_"),
        contrasts = c("conditiontreatment - conditioncontrol")
      ) %>%
    ...
    tmp_de_res <-  
      ...
      test_differential_abundance(
        ...
        contrasts = list(c("condition", "treatment", "control"))
        ) %>% 
      as_tibble() %>% 
...
```
```{r function_de_cycle}
## Function: Perform all DE analysis methods on given tables/normalisations including different phases and cycles
compute_DE_genes_cycle <- function(expr_list, phase_value = "normal"){ 
  
  for (i in 1:length(expr_list_cycle)){  
    
    expr_source <- names(expr_list)[i]
    
    print(paste0("Now processing ", expr_source))
...
```{r}
#de_obj.1 <- 
  counts %>% filter(cycle == 1 & phase == "normal") %>% 
      test_differential_abundance(
        ~ 0 + condition +  lane,
        method = "edgeR_quasi_likelihood",
        prefix = paste0("counts", "_", "edgerQLT_"),
        contrasts = c("conditiontreatment - conditioncontrol")
        
      ) #%>% as_tibble()

de_obj.1 %>% as_tibble() %>% 
  filter("counts_edgerQLT_PValue___conditiontreatment - conditioncontrol" < 1.0)
```
...
## Compute DE based on each cycle
```{r}
expr_list_cycle <- list("raw1" = counts %>% filter(cycle == 1), 
                        "combat1" = combat_counts %>% filter(cycle == 1), 
                        "hkg1" = hkg_counts %>% filter(cycle == 1), 
                        "combat_hkg" = hkg_combat_counts %>% filter(cycle == 1), 
                        "raw2" = counts %>% filter(cycle == 2), 
                        "combat2" = combat_counts %>% filter(cycle == 2), 
                        "hkg2" = hkg_counts %>% filter(cycle == 2), 
                        "combat_hkg3" = hkg_combat_counts %>% filter(cycle == 2),
                        "raw3" = counts %>% filter(cycle == 3), 
                        "combat3" = combat_counts %>% filter(cycle == 3)#, 
                        #"hkg3" = hkg_counts %>% filter(cycle == 3), 
                        #"combat_hkg3" = hkg_combat_counts %>% filter(cycle == 3)
                  )
```

```{r}
de_norm_cycle <- compute_DE_genes_cycle(expr_list_cycle, phase = "normal")
comp_de_norm <- compute_composite_DE_results(de_norm)
```
...
The first method was using a curated structured database of biofilm-related
proteins: https://biofilms.biosim.pt/. This method failed as it did not match
against any of the genes within our dataset.

Next, we use keywords related to biofilm formation and search them against our
gene annotations. These keywords include: "Biofilm", "Quorum sensing",
"Adhesion", "Exopolysaccharide", "Motility", "Flagella", "Pili", "Extracellular"
and "Matrix". *DISCLAIMER*: These keywords were collected via a shallow literature 
search and with ChatGPT. 

The keyword based search yielded a total of 28 genes of which a majority of them
were related to "pili" and "flagella". Out of those, nine of them appeared to be
significant and had an absolute fold-change \leq 2. Those genes appear to be
downregulated in the treatment samples (@fig-biofilm_genes).

```{r biofilm_symbols, eval = F}
...
MAG_annotation %>% filter(gene_symbol %in% biofilm_genes) %>% 
  filter(!is.na(gene_symbol)) %>% inner_join(comp_de_norm, by = "feature") %>% 
  left_join(combat_counts %>% as_tibble(), by = "feature") %>% 
  filter(counts > 1) %>% 
# make faceted stripchart
  ggplot(aes(x = condition, y = counts + 1, fill = condition, group = condition)) +
...
```{r}
#| label: fig-biofilm_genes
#| fig-cap: "Strip-violin-chart of the most significantly expressed biofilm-related genes." 
#| fig-height: 7

annot_dat <- 
  MAG_annotation %>% 
  filter(grepl(pattern, gene_function, ignore.case = TRUE)) %>% 
  inner_join(comp_de_norm, by = "feature") %>% 
  left_join(combat_counts %>% as_tibble(), by = "feature") %>% 
  ...
  select(-is_unique) %>% 
  select(feature, gene_name, composite_p_val_fisher_fdr, condition, counts) %>%
  distinct() %>%
  mutate(group1 = "control", group2 = "treatment",
         composite_p_val_fisher_fdr = round(composite_p_val_fisher_fdr, 5)) %>%
  group_by(feature) %>% 
...
MAG_annotation %>% 
  ...
  geom_boxplot(colour = "gray75", alpha = 0.25, width = 0.1) +
  geom_jitter(aes(colour = condition, shape = cycle), alpha = 0.25, ) +
  facet_wrap(~gene_name) +
  scale_y_log10() +
  theme_minimal() +
  add_pvalue(data = annot_dat, label = "Fisher FDR = {composite_p_val_fisher_fdr}", y.position = "ypos", bracket.nudge.y = 0.1) +
